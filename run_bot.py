...
if __name__ == "__main__":
    ...
    with open('cfx-data3.csv', newline='') as File:  
        ...
        for j2 in range(8 , 9):
            for j3 in range(1 , 2):
                with logger.Profiler() as p:
                    ...
                    k_price_order_stop = 1.0
                    k_price_estimated = 1 + j2 / 100 #.1 # 1.05 - 1.15 На сколько допустимо превысить рассчетную цену доходности.
                    time_start_order = j3 * 30 # 10 - 100
                    ...
                    if p > 0:
                        log.warning(f"|j1 = {j1} |k_price_estimated = {j2} |time_start_order = {j3/2} |minumum_BTC={start_balance - nice.minimum_balance_BTC:2.8f} |end=|{nice.balance_BTC:2.8f}|percent = {p:3.3f}%")
                    else:
                        log.error(f"|j1 = {j1} |k_price_estimated = {j2} |time_start_order = {j3/2} |minumum_BTC={start_balance - nice.minimum_balance_BTC:2.8f} |end=|{nice.balance_BTC:2.8f}|percent = {p:3.3f}%")